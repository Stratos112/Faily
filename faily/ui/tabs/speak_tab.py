...
def _build_expression(char_state: list[str]):
    """Two-step pipeline: expression engine → voice conversion. Returns (refresh, select)."""
    ...
    with ui.grid(columns="2fr 3fr").classes("w-full h-full gap-0"):
        ...
        pb, ml, mp, st, _, _, ath = output_panel(
            "vc", get_char_name=lambda: char_state[0] if char_state[0] != _NO_CHAR else None
        )
...
def _build_character(char_state: list[str]):
    """CHARACTER sub-tab: TTS via trained RVC model. Returns refresh."""
    ...
    async def _generate():
        ...
        try:
            ...
            path = await ni_run.io_bound(
                speak_generate, text, char["rvc_model"], SPEAK_OUTPUT_DIR,
    ...
    with ui.grid(columns="2fr 3fr").classes("w-full h-full gap-0"):
        ...
        pb, ml, mp, st, _, _, ath = output_panel(
            "speak", get_char_name=lambda: char_state[0] if char_state[0] != _NO_CHAR else None
        )
        _out.update(progress_bar=pb, model_loader=ml, main_player=mp, status=st, add_to_history=ath)
...
def _build_oneshot(char_state: list[str]):
    """ONE SHOT sub-tab: pick character → use ref chain → vc_generate. Returns refresh."""
    _progress: list[float] = [0.0]
...
def build_speak_tab():
    ...
    _r_exp:  list = [lambda: None]
    _s_exp:  list = [lambda n: None]
    _r_char: list = [lambda: None]
    ...
    with ui.column().classes("w-full h-full gap-0"):
        ...
        with ui.tab_panels(inner_tabs, value=exp_tab).classes("w-full flex-grow"):
            with ui.tab_panel(exp_tab):
                _r_exp[0], _s_exp[0] = _build_expression(_exp_state)
            with ui.tab_panel(char_tab):
                _r_char[0] = _build_character(_char_state)
            with ui.tab_panel(shot_tab):
                _r_shot[0] = _build_oneshot(_shot_state)
